...
class Calendar 
{
	...
	public function sHowCalendar()
	{
			...
			if($firstday == 1)
			{
				for($i=1,$dayslastmonth-=6; $i <= 7; $i++,$dayslastmonth++) echo '<button disabled="disabled">',sprintf("%02d",$dayslastmonth),'</button>';
				echo '<br>';
				$diferentdays=$i;
			}
			else
			{
				for($i=1,$dayslastmonth-=($firstday-2); $i < $firstday; $i++,$dayslastmonth++) echo '<button disabled="disabled">',sprintf("%02d",$dayslastmonth),'</button>';
				$diferentdays=$i;
			}
			/*
				$today = strtotime(date("Y-m-d"));
				$expiration_date = strtotime(2010-02-24);
				echo "<br>";
		...
		//		if($expiration_date < $today) echo sprintf("<button type='submit' disabled='disabled'>%02d</button>",$day); 
				if((substr($_POST['date'],8,2) ==  $day && !isset($_POST['changedate']) && $_POST['callendar']=='send') || ($this->day == $day))
				{ 
					echo '<button style="background-color:red" name=date'.sprintf("%02d-%02d-%02d",$this->year,$this->month,$day).'type="submit" >'; //błąd brak nazwy przycisku i wartości
				...
				{
					echo sprintf("<button type=\"submit\" name=date value='".date("Y-m")."-%02d'>",$day); 
					echo '<font color=red>';
					echo sprintf("%02d",$day);
					echo '</font>';
					echo '</button>';
				}else echo sprintf("<button type=\"submit\" name=date value='%02d-%02d-%02d'>%02d</button>",$this->year,$this->month,$day,$day);
				if(($i%7)==0) echo '<br>';
			}
