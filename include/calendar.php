...
class Calendar 
{
		...
		if(isset($_SESSION['cmonth'])) //sprawdzam czy istnieje cmonth w danych sesji
		{
			...
			if(isset($_POST['changedate']))
			{
				if($_POST['changedate'] == 'next')
				{
					...
					$_POST['date']=$_SESSION['cyear'].sprintf("-%02d-%02d",$_SESSION['cmonth'],1);
				}
	...
	public function sHowCalendar()
	{
		...
		//		if($expiration_date < $today) echo sprintf("<button type='submit' disabled='disabled'>%02d</button>",$day); 
				if(isset($_POST['callendar'])) $testvalue=$_POST['callendar'];
				else $testvalue=false;
				if((substr($_POST['date'],8,2) ==  $day && !isset($_POST['changedate']) && $testvalue=='send' ) || $this->checkBacklight($day,$this->month,$this->getYear()) == true  )
				{ 
