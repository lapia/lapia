...
{	
	...
	public function ShowForm($phpfile)
	{
			...
			$this->ShowList("areayear",$this->y-1,2019,$this->y); //$this->y+1
			echo "<br>Time";
			$this->ShowList("areatime",0,23,$this->time,':00');
			echo "<br>Duration";
	...
	private function ShowList($name,$start,$stop,$test=-1,$sufix="")
	{
		$menu.= "<select name='".$name."' onChange='submitManuaChosers()'>";
			...
			{
				if($test== $i) $menu.= "<option selected='selected' value='".sprintf("%02d",$i)."'>".sprintf("%02d",$i).$sufix."</option>";
	...
	public function setPassingTime($passingtime){ $this->passingtime=$passingtime;}
}
?>
